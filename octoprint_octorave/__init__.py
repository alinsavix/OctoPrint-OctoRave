...
class OctoravePlugin(octoprint.plugin.EventHandlerPlugin,
	...
	def notify_event(self,eventID,data={}):
		...
		# self._logger.info(f"ZOTZOTZOT: {data}")
		...
		try:
			...
			message = tmpConfig["message"] + \
				"""\r\n:sos: **Octorave Warning**""" + \
				"""\r\n The variable `{""" +  error.args[0] +"""}` is invalid for this message: """ + \
				"""\r\n Available variables: `{""" + '}`, `{'.join(list(data)) +"}`"
		finally:
			fn = data["path"].replace(".gcode", ".png") + "?" + \
                    "{:%Y%m%d%H%M%S}".format(datetime.datetime.now())
			return self.send_message(eventID, message, tmpConfig["with_snapshot"], fn)
    ...
    # Prusa thumbnail URL e.g.: http://localhost:5000/plugin/prusaslicerthumbnails/thumbnail/calicat_-_0.20mm_QUALITY_-MK3_-_PETG_-_Polymaker_Polylite_-_Grey_-_7.42977g.png?20210331153945
    # also flask.url_for("plugin.prusaslicerthumbnails.thumbnail")
	def send_message(self, eventID, message, withSnapshot=False, thumbnail=False):
		...
		self._logger.debug(f"ZOT thumbnail: {thumbnail}")
		if withSnapshot and eventID == "printing_started" and thumbnail:
			baseUrl = "http://localhost:5000/plugin/prusaslicerthumbnails/thumbnail"
			# fn=["path"].replace(".gcode", ".png") + "?" + "{:%Y%m%d%H%M%S}".format(datetime.datetime.now())
			snapshotUrl=f"{baseUrl}/{thumbnail}"
			using_thumbnail = True
			# self._logger.debug(f"thumbnail URL: {snapshotUrl}")
		...
		if 	withSnapshot and snapshotUrl is not None and "http" in snapshotUrl :
			try:
				...
				if snapshotCall :
					...
					if not using_thumbnail and (mustFlipH or mustFlipV or mustRotate):
...
def __plugin_load__():
	...
	__plugin_implementation__ = OctoravePlugin()
